...
$tabla="atencion";
$campos="DISTINCT  atencion.id_atencion,atencion. cedula_paciente as cedula_paciente, atencion.cedula_medico, atencion.id_usuario,atencion.id_tipo, estado_paciente_inicio, descripcion, fecha_inicio, fecha_fin,tratamiento.fecha_registrado as fecha_registrada ,paciente.nombre_paciente,medico.nombre_medico,case
...
$where="where atencion.id_tipo in (1,2,3,4,5) and atencion.cedula_medico='$cedula_usu'   ORDER BY `tratamiento`.`id_tratamiento` DESC";
$inner="
...
INNER JOIN tratamiento ON tratamiento.id_atencion=atencion.id_atencion
";
...
while ($row=$consulta->fetch()) {
    $button='<button onclick="imprimir_receta_medica(\''.$row["id_atencion"].'\')" title="">
